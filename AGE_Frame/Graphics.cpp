...
void AGE_Frame::PrepGraphicSearch()
{
    ...
    for (size_t loop = 0; loop < 2; ++loop)
    {
        ...
        else if (label.compare("Frame Duration") == 0)
            GraphicFilterFunctions.push_back([this](genie::Graphic *graphic_ptr)
        {
            return "FD " + FormatFloat(graphic_ptr->FrameDuration);
        });
...
void AGE_Frame::OnGraphicSelect(wxCommandEvent &event)
{
    ...
    SetStatusText(wxString::Format("Selections: %zu    Selected sprite: %d",
...
void AGE_Frame::OnDrawGraphicSLP(wxPaintEvent &event)
{
    ...
    else if(currentDisplay == SHOW::UNIT)
    {
        ...
        {
            dc.DrawLabel("No sprite " + FormatInt(museum.datID), wxRect(text_pos, text_pos, 100, 40));
            return;
        }
